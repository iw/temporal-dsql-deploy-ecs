...
# -----------------------------------------------------------------------------
# Uses the same instance type as Temporal nodes (m7g.large/m7g.xlarge) for
...
resource "aws_autoscaling_group" "benchmark" {
  ...
  max_size            = var.benchmark_max_instances
...
resource "aws_ecs_capacity_provider" "benchmark" {
  ...
  auto_scaling_group_provider {
    ...
    managed_scaling {
      maximum_scaling_step_size = var.benchmark_max_instances
