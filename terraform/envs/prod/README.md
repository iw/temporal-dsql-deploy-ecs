...
Production environment for Temporal on ECS with Aurora DSQL persistence. This environment uses production-grade resources for reliability and performance, with the benchmark module disabled.
...
key          = "temporal-dsql/prod/terraform.tfstate"
region       = "eu-west-1"
encrypt      = true
use_lockfile = true
```
...
Then update the required values (DSQL endpoint, cluster ARN, and image URIs). The example file contains representative defaults for a WPS 100 production environment with full documentation of each setting.
...
```bash
# From the project root
./scripts/scale-services.sh prod up
```
...
The prod environment uses production-grade resources:
...
| Availability Zones | 3 | eu-west-1a, 1b, 1c |
| History Replicas | 6 | 2048 CPU, 8192 MB each |
| History Shards | 4096 | |
| Matching Replicas | 4 | 1024 CPU, 2048 MB each |
| Frontend Replicas | 3 | 1024 CPU, 4096 MB each |
| Worker Replicas | 2 | 512 CPU, 1024 MB each |
| UI Replicas | 1 | 256 CPU, 512 MB |
| OpenSearch Instance Type | m6g.large.search | |
| OpenSearch Instance Count | 3 | HA configuration |
| Loki | 1 replica | 512 CPU, 1024 MB, 30 day retention |
| Grafana | 1 replica | 256 CPU, 512 MB |
| Alloy Sidecars | Per service | Metrics + logs collection |
| Log Retention | 30 days | CloudWatch logs for observability |
| Benchmark | Disabled | Use bench environment |

## Production Considerations

### High Availability

- Deployed across 3 availability zones by default
- OpenSearch uses 3 nodes for HA
- ECS services spread across multiple instances
...
- Alloy sidecars for metrics/log collection

### Benchmarking

Benchmark infrastructure is **NOT** deployed in the prod environment. Use the bench environment for performance testing:
...
```bash
# Scale all services up (production replicas)
./scripts/scale-services.sh prod up

# Scale all services down (0 replicas)
./scripts/scale-services.sh prod down
...
1. Query Loki logs: `./scripts/query-loki-logs.sh prod frontend`
...
./scripts/query-loki-logs.sh prod frontend

# Query history logs with custom time range
./scripts/query-loki-logs.sh prod history -t 2h

# Query matching logs with filter
./scripts/query-loki-logs.sh prod matching -f "error"
```
