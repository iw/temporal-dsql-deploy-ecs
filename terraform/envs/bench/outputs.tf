...
output "temporal_ui_port_forward_command" {
  ...
  value       = <<-EOT
    ...
    aws ssm start-session \
      --target ecs:${module.ecs_cluster.cluster_name}_$TASK_ID_$RUNTIME_ID \
...
output "run_benchmark_command" {
  description = "Command to run a benchmark test"
  value = (
    var.benchmark_enabled ? <<-EOT
    # Run benchmark with default settings (100 WPS for 5 minutes)
    aws ecs run-task \
      --cluster ${module.ecs_cluster.cluster_name} \
      --task-definition ${module.benchmark[0].task_definition_arn} \
      --capacity-provider-strategy capacityProvider=${module.benchmark[0].capacity_provider_name},weight=1 \
      --network-configuration "awsvpcConfiguration={subnets=[${module.vpc.private_subnet_ids[0]}],securityGroups=[${module.benchmark[0].security_group_id}],assignPublicIp=DISABLED}" \
      --overrides '{"containerOverrides":[{"name":"benchmark","environment":[{"name":"BENCHMARK_RATE","value":"100"},{"name":"BENCHMARK_DURATION","value":"5m"}]}]}' \
      --region ${var.region}
    EOT
    : "Benchmark is not enabled in this environment"
  )
}

output "scale_benchmark_workers_command" {
  description = "Command to scale benchmark workers"
  value = (
    var.benchmark_enabled ? <<-EOT
    # Scale benchmark workers up (e.g., to 30 workers for 100 WPS)
    aws ecs update-service \
      --cluster ${module.ecs_cluster.cluster_name} \
      --service ${module.benchmark[0].worker_service_name} \
      --desired-count 30 \
      --region ${var.region}
    
    # Scale benchmark workers down
    aws ecs update-service \
      --cluster ${module.ecs_cluster.cluster_name} \
      --service ${module.benchmark[0].worker_service_name} \
      --desired-count 0 \
...
output "all_service_names" {
  ...
  value = {
    ...
    benchmark_worker  = var.benchmark_enabled ? module.benchmark[0].worker_service_name : null
  }
}
...
output "deployment_summary" {
  ...
  value       = <<-EOT
    ...
    ============================================================
    TEMPORAL ECS DEPLOYMENT SUMMARY (BENCH ENVIRONMENT)
    ============================================================
    ...
    - Grafana:           ${module.observability.grafana_service_name}
    ${var.benchmark_enabled ? "- Benchmark Worker:  ${module.benchmark[0].worker_service_name}" : ""}
    ...
    - Benchmark Enabled: ${var.benchmark_enabled}
    - Max Benchmark Instances: ${var.benchmark_max_instances}
    ${var.benchmark_enabled ? "- Benchmark Capacity Provider: ${module.benchmark[0].capacity_provider_name}" : ""}
