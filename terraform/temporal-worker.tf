...
# -----------------------------------------------------------------------------
# This file creates the Temporal Worker service:
# - Task definition with ARM64 (Graviton) architecture
# - ECS service with Service Connect (client-only mode)
# - Port mappings: 7239 (gRPC), 6939 (membership), 9090 (metrics)
# 
...
#
# Worker is a client-only service - it connects to Frontend but doesn't
# expose discoverable endpoints for other services.
#
# Requirements: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.9, 5.1, 5.2, 5.3, 5.5, 5.8, 5.10
# -----------------------------------------------------------------------------
  ...
  container_definitions = jsonencode([
    {
      ...
      # Port mappings for Worker service
      # Requirements: 4.4
      portMappings = [
        {
          containerPort = 7239
...
resource "aws_ecs_service" "temporal_worker" {
  ...
  network_configuration {
    ...
    security_groups  = [aws_security_group.ecs_instances.id, aws_security_group.temporal_worker.id]
  ...
  # Service Connect configuration - client-only mode with metrics endpoint
  # Worker connects to Frontend but doesn't expose discoverable gRPC endpoints
  # However, we expose metrics for ADOT scraping
  # Requirements: 5.8
  service_connect_configuration {
    ...
    service {
      port_name      = "metrics"
      discovery_name = "temporal-worker-metrics"
      client_alias {
        port     = 9090
        dns_name = "temporal-worker-metrics"
      }
    }
  }
  ...
  depends_on = [
    aws_ecs_task_definition.temporal_worker,
